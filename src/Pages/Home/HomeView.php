...
class HomeView extends PagesWithExceptionView
{
    public $isLoggedIn;
    ...
    private function renderTitularBlock()
    {
        return '<div class="p-4 p-md-5 mb-4 text-white rounded bg-dark">
                    <div class="col-md-6 px-0">
                        <h1 class="display-4 fst-italic">'.$this->model->titlePost->heading.'</h1>
                        <p class="lead my-3">'.$this->model->titlePost->shortDescription.'</p>
                        <p class="lead my-3">'.$this->model->titlePost->user->name.'</p>
                        '.( ($this->isLoggedIn) ? '<p class="lead mb-0"><a href="/post?postId='.$this->model->titlePost->id.'">go to reading</a></p>' : '').'
                    </div>
                </div>';
    }
    ...
    private function renderPostsBlock()
    {
        ...
        {
            if($post['published'] === 1) {
                if($post['id'] !== $this->model->titlePost->id) {
                    $result .= '
                        <div class="col-md-6">
                            <div class="row g-0 border rounded overflow-hidden flex-md-row mb-4 shadow-sm h-md-250 position-relative">
                                ...
                                <div class="col-auto d-none d-lg-block">
                                    <img src="'.$post['pathToImg'].'" width="200" height="250">
                                </div>
                            </div>
                        </div>';
                }
